...
function Home() {
  ...
  const getPhoneInfo = async () => {
    setIsLoading(true);
    try {
      const info = await window.signal.getDevices(
        "shell getprop ro.product.bootimage.marketname",
      );
      if (
        info &&
        !info.includes("device offline") &&
        !info.includes("not found")
      ) {
        setPhoneInfo(info.trim());
      } else {
        setPhoneInfo("No device connected");
      }
    } catch {
      setPhoneInfo("Error connecting to ADB");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        ...
        <div className="flex items-center justify-between w-full p-4 border rounded-2xl bg-slate-900/50 border-slate-700/50">
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-lg ${phoneInfo.includes("connected") || !phoneInfo.includes("No device") ? "bg-primary-500/20 text-primary-400" : "bg-red-500/20 text-red-400"}`}
            >
              {phoneInfo.includes("Searching") || isLoading ? (
